...
class Canvas extends React.Component{
    ...
    updateAnimationState() {
        if (this.state.frameCounter >= (this.state.width / 5.0)/ 2.0 - 0.5){
            ...
            if(this.state.stageCounter >= 2){
                ...
                this.setState(prevState => ({stageCounter: prevState.stageCounter + 1 }))
            }
        }
    ...
    drawNeurons(c, width, height){
        ...
        var outputTop = new Circle(ouputLayerXPos, yPos - outputLayerVerticalSpacing, 0, 0, 13, "white", "#3FAAA2");
        var outputBottom = new Circle(ouputLayerXPos, yPos + outputLayerVerticalSpacing, 0, 0, 13, "white", "#3FAAA2");
        outputTop.draw(c);
        outputBottom.draw(c);
    }
    ...
    componentDidUpdate(){
        ...
        for (var j = 0; j < this.state.allSignals[this.state.stageCounter].length; j++) {
    ...
    render(){
        return(
            <canvas width= {600}  height= {325} ref={this.canvasRef} />
        )
    }
}
