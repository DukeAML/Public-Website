...
function drawPipe(x, y, life, ttl, width, hue) {
  ctx.a.save();
  ctx.a.strokeStyle = `hsla(${hue},20%,50%,${fadeInOut(life, ttl) * 0.125})`;
  ctx.a.fillStyle = `hsla(${hue},20%,50%,${fadeInOut(life, ttl) * 0.125})`;
  ctx.a.beginPath();
...
class AnimatedBg extends React.Component {
  constructor(props) {
    super(props);
    this.state = { angle: 0 };
    this.canvasRef = Recat.createRef();
    this.state = {
      allSignals: [[], [], []],
      frameCounter: 0,
      stageCounter: 0,
