...
class StatisticsPool {
  ...
  static async fetchClusterNames() {
    try {
      const startTime =
        new Date(new Date().getTime() - 31 * 60 * 1000).getTime() / 1000;
  ...
  static async fetchStatisticPool(statisticNames, clusters) {
    try {
      ...
      for (const statisticName of statisticNames) {
        const startTime =
          new Date(new Date().getTime() - 12 * 60 * 60 * 1000).getTime() / 1000;
        const endTime = new Date().getTime() / 1000;

        let statisticsResponse;

        const cachedTime = JSON.parse(
          localStorage.getItem('cachedGPUStatisticTime'),
        );

        if (
          Date.now() / 1000 - cachedTime < 5 * 60 &&
          localStorage.hasOwnProperty(`cachedGPUStatistics_${statisticName}`)
        ) {
          statisticsResponse = JSON.parse(
            localStorage.getItem(`cachedGPUStatistics_${statisticName}`),
          );
        } else {
          statisticsResponse = await gpustatistics.get(`/${statisticName}`, {
            params: { startTime, endTime, stepSize: '30m' },
          });

          localStorage.setItem(
            `cachedGPUStatistics_${statisticName}`,
            JSON.stringify(statisticsResponse),
          );
          localStorage.setItem(
            'cachedGPUStatisticTime',
            JSON.stringify(endTime),
          );
        }

        let matchingClusters = Object.entries(statisticsResponse.data).filter(
          ([key, val]) => clusters.includes(key),
        );

        console.log(matchingClusters);

        matchingClusters.forEach((matchingCluster) => {
          const values = matchingCluster[1].values.map(
            (value) => value / 1073741824,
          );
          const statistic = new Statistic(
            statisticName,
            matchingCluster[0],
            matchingCluster[1].times,
            values,
          );
  ...
  convertToChartData = () => {
    ...
    const data = {
      ...
      datasets: this.statistics.map((statistic) => {
        ...
        const statisticConfigKey = Object.keys(Statistic.statistics).find(
          (key, index) => Statistic.statistics[key].name === statistic.name,
        );
