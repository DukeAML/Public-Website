...
class GPUClusterChartLegend extends React.Component {
    ...
    render() {
        return (
            <div id="cluster-chart-legend" style={{display: 'flex', justifyContent: 'space-around', flexWrap: 'wrap'}}>
                {
                    Object.keys(this.props.lineTypes).map(cluster => {
                        return <div key={cluster} className="cluster-chart-legend-element" style={{display: 'flex'}}>
                            <canvas id={`canvas-${cluster}`} style={{width: '50px', height: '50px'}}></canvas>
                            <p>{cluster}</p>
                        </div>
                    })
                }
